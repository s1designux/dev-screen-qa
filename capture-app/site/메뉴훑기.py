...
_잠금 = threading.Lock()
# "까닭" 은 하다가 안 된 것(붉게 보인다), "아직" 은 사람이 적을 것이 남은 것(그냥 안내다).
...
def 돌만한가(작업):
    ...
    못 도는 것은 **아직 적을 것이 남은 것**이지 잘못된 것이 아니다 — 안내 말로 돌려준다.
    """
    if (작업.get("유형") or "") not in (매체.PC웹, 매체.모바일웹):
        return False, "웹이 아닙니다."
    if not (작업.get("기본주소") or "").strip():
        return False, "기본 주소를 적고 'IA 읽어오기'를 누르세요."
    tag = _tag(작업)
    if 들어가는길.로그인쓰나(tag):
        것 = 들어가는길.계정읽기(tag)
        if not (것["아이디"] and 것["비밀번호"]):
            return False, "시험 아이디·비밀번호를 적고 'IA 읽어오기'를 누르세요."
    return True, ""
...
def 자동시작(작업, 다시=False):
    ...
    된다, 안내 = 돌만한가(작업)
    if not 된다:
        # 적을 것이 남았을 뿐이다 — 붉은 '읽지 못했습니다' 로 놀래지 않는다.
        _고치기(까닭="", 아직=안내, 말="", 진행중=False)
        return False
    기억 = 기억읽기(앱이름)
    if 기억 and not 다시:
        메뉴들 = 기억.get("메뉴") or []
        _고치기(앱이름=앱이름, 때=기억.get("때", ""), 까닭="", 아직="", 진행중=True,
