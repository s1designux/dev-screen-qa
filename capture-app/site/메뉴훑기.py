...
def 한바퀴(작업):
    ...
    with sync_playwright() as 연장:
        ...
        try:
            ...
            더볼것 = 한단계더볼것
            i = 0
            while i < len(차례) and i < 메뉴많아야:
                하나 = 차례[i]
                i += 1
                _고치기(지금=i, 전부=len(차례), 말="메뉴를 읽는 중", 읽는것=하나["이름"])
                try:
                    if _다듬기(쪽.url) != 하나["주소"]:
                        쪽.goto(하나["주소"], wait_until="load", timeout=20000)
                    webshot._가라앉기(쪽, 1.5)
                    사진 = 쪽.screenshot(full_page=True)
                    제목 = 쪽.evaluate("() => (document.querySelector('h1,h2')?.innerText "
                                    "|| document.title || '').trim().slice(0, 40)")
                except Exception:
                    continue
                _메뉴더하기(메뉴들, {"이름": 하나["이름"], "제목": 제목, "주소": 하나["주소"],
                                 "주소길": _주소길(하나["주소"], 바탕),
                                 "지문": 지문(쪽, 사진)})
                if 더볼것 > 0 and len(차례) < 메뉴많아야:
                    더볼것 -= 1
                    차례 += 링크모으기(쪽, 바탕, 이미본것)
                    차례 = 차례[:메뉴많아야]
                # 읽는 대로 바로 짚어 준다 — 다 끝날 때까지 기다리지 않게
                _고치기(짝=_짝맞추기(시안지문들, 메뉴들),
                      메뉴=[{"이름": m["이름"], "주소길": m["주소길"]} for m in 메뉴들],
                      전부=len(차례))
        finally:
            webshot.브라우저끄기(브라우저)
