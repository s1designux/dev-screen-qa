...
def 찍기(tag, 결과폴더, 이름짓기=None):
    ...
    with sync_playwright() as 연장:
        ...
        try:
            ...
            # 로그인 창만 찍힌다. 그래서 **찍기 전에 이 창에서 한 번** 들어가 둔다(찍지 않는다).
            if 들머리필요:
                본것 = 들어가는길.웹으로(쪽, tag, 기다림)
                if 본것["됨"] is False:
                    print(f"{계정확인.막힘표}{'(잠김)' if 본것['잠김'] else ''}: {본것['까닭']}",
                          flush=True)
                    print("   로그인이 되지 않아 한 장도 찍지 않았습니다 — "
                          "아이디·비밀번호를 고치고 다시 시작해 주세요.", flush=True)
                    for 화면 in tag["화면"]:
                        실패.append({"화면이름": 화면["이름"],
                                   "까닭": f"로그인하지 못했습니다 — {본것['까닭']}"})
                    tag = dict(tag, 화면=[])
                elif 본것["됨"] is True:
                    print("  로그인 화면은 안 찍지만 들어가는 길은 지났습니다 — "
                          "이 창 그대로 이어 찍습니다\n", flush=True)
                else:
                    print(f"  들어가는 길을 지나지 못했습니다 — {본것['까닭']}\n", flush=True)
            ...
            for i, 화면 in enumerate(tag["화면"], 1):
                ...
                if not _살아있나(브라우저, 쪽):
                    ...
                    # 창을 새로 열면 로그인도 풀린다 — 들어가는 길을 한 번 더 지난다.
                    if 들머리필요:
                        다시 = 들어가는길.웹으로(쪽, tag, 기다림)
                        print("    · 로그인을 다시 했습니다" if 다시["됨"] is True
