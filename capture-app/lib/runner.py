...
def 대본쓰기(tag, 한묶음, 사진이름들, 대본폴더, 순번, 계정=None, 자리=None, 들머리줄=None):
...
def 다시찍기(tag, 묶음차례, 계정, 손파일, 결과폴더, 임시, 들머리줄=None):
...
def 한번에찍기(tag, 결과폴더):
    ...
    # 로그인 화면을 고르지 않고 홈·메뉴만 찍을 때 — 로그인은 '들어가는 길'로 따로 지난다.
    들머리줄, 들머리알림 = 들어가는길.앱줄(tag, 계정)
    if 들머리알림:
        print(들머리알림 + "\n", flush=True)
    elif 들머리줄:
        print("로그인 화면은 안 찍지만, 화면마다 들어가는 길을 먼저 지납니다.\n", flush=True)
...
def main():
    ...
    # 웹에서 '들어가는 길'을 지날 때는 미리 해 보지 않는다 — 어차피 바로 뒤에 같은 로그인을 한다.
    # 두 번 틀리면 잠김(5회)에 두 칸을 그냥 쓴다.
    미리안함 = (tag["플랫폼"].strip().lower() == "web" and 들어가는길.필요한가(tag))
    본것 = 계정확인.못해봄("바로 뒤에 들어가는 길에서 해 봅니다.") if 미리안함 else 계정확인.확인(tag)
    if 본것["됨"] is False:
