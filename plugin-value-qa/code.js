...
//  역할: 사용자가 고른 "정답 프레임" 안의 이름표(레이어) 값을
...
// Inter 굵기 이름 → 숫자 (개발쪽 fontWeight와 맞추기 위함)
...
function fillRgb(node) {
  ...
  for (var i = 0; i < fs.length; i++) {
    ...
    if (f.visible !== false && f.type === 'SOLID') {
      var c = f.color; return 'rgb(' + to255(c.r) + ', ' + to255(c.g) + ', ' + to255(c.b) + ')';
    }
  }
  return null;
}
function strokeRgb(node) {
  ...
  for (var i = 0; i < ss.length; i++) {
    ...
    if (s.visible !== false && s.type === 'SOLID') {
      var c = s.color; return 'rgb(' + to255(c.r) + ', ' + to255(c.g) + ', ' + to255(c.b) + ')';
    }
  }
  return null;
}

// 검수 대상 이름표인가? (그룹/요소 꼴 "header/logo") — 보조 라벨(/label)과 루트는 제외
function isQaName(name) {
  if (!name) return false;
  if (name.indexOf('/') < 0) return false;
  if (name.lastIndexOf('/label') === name.length - 6) return false;
  return true;
}
...
function readElement(node, rootBox) {
  ...
  if (isText) {
    ...
    style = {
      color: fillRgb(node) || 'rgb(0, 0, 0)',
      backgroundColor: 'rgba(0, 0, 0, 0)',
      fontSize: r1(node.fontSize),
      fontWeight: WEIGHT[sty] || 400,
      fontFamily: fam,
      lineHeight: 0,
    ...
    // 원(타원)은 개발화면에서 border-radius 999(둥근)로 잡히므로 맞춰줌
    var radius = (node.type === 'ELLIPSE') ? 999
      : (typeof node.cornerRadius === 'number' ? node.cornerRadius : 0);
    var topStroke = 0;
    if (node.strokes && node.strokes.length) {
      topStroke = (node.strokeTopWeight != null) ? node.strokeTopWeight : (node.strokeWeight || 0);
    }
    style = {
      color: 'rgb(31, 41, 55)',
      backgroundColor: fillRgb(node) || 'rgba(0, 0, 0, 0)',
      fontSize: 16, fontWeight: 400, fontFamily: 'Inter',
      lineHeight: 0,
      borderRadius: r1(radius),
      borderWidth: r1(topStroke),
      borderColor: strokeRgb(node) || 'rgb(31, 41, 55)',
  ...
  return {
    qa: node.name, role: node.type, box: box, style: style,
    contentZone: node.name.indexOf('content/') === 0, isText: isText
  };
}
...
function readDesign(root) {
  ...
  var out = [];
  function walk(node) {
    if (node.id !== root.id && node.visible !== false && isQaName(node.name)) {
  ...
  walk(root);
  return {
    meta: { label: 'design', source: 'figma', frameName: root.name, artboardWidth: r1(rootBox.width), toolVersion: '1.0' },
    elements: out
  };
}

// ---- 선택 알림: 정답 프레임 후보를 UI에 표시 ----
...
figma.ui.onmessage = async function (msg) {
  try {
    if (msg.type === 'init') {
      ...
      var sel = figma.currentPage.selection;
      if (!sel || sel.length === 0) {
        figma.ui.postMessage({ type: 'design-read', error: '피그마에서 정답이 될 프레임(화면)을 먼저 선택해 주세요.' });
        return;
      }
      var root = sel[0];
      if (!('children' in root) || !root.absoluteBoundingBox) {
        figma.ui.postMessage({ type: 'design-read', error: '이 항목은 프레임이 아니에요. 화면 전체 프레임을 골라주세요.' });
        return;
      }
      var data = readDesign(root);
      if (data.elements.length === 0) {
        figma.ui.postMessage({ type: 'design-read', error: '읽을 이름표가 없어요. 레이어 이름을 개발쪽 이름표(예: header/logo)와 똑같이 지어주세요.' });
        return;
      }
      ...
      figma.ui.resize(Math.max(380, msg.width | 0), Math.max(480, msg.height | 0));
    }
