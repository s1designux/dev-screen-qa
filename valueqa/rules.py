...
def 토큰밖(요소들, 토큰, 허용차=0):
    ...
    for c in 모음.values():
        차이 = c["가까운토큰"]["차이"]
        # 차이가 클수록 '토큰 밖' 이 뚜렷하다. 아주 가까우면(1~2) 사람이 보고 넘길 수 있게 신뢰도를 낮춘다.
        c["신뢰도"] = round(min(1.0, 0.6 + 0.1 * min(4, 차이)) if c["속성"] not in ("color", "backgroundColor", "borderColor")
                        else min(1.0, 0.55 + 0.03 * min(15, 차이)), 3)
        후보.append(c)
