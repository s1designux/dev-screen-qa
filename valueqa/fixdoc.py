...
def _규정본문(L, 장번호, 토큰항목, 규격항목, 알아봄들, 여러화면):
    ...
    if 토큰항목:
        ...
        L.append("")
        L.append("| 번호 | %s고칠 것 | 지금 (개발) | 바꿀 토큰 | 어디 (개발 화면 기준) |" % 화면칸)
        L.append("|---|%s---|---|---|---|" % 화면선)
        for it in 토큰항목:
...
def _본문(L, 항목들, 구조들, 주의들, 여러화면, 토큰항목=None, 규격항목=None, 알아봄들=None):
    토큰항목 = 토큰항목 or []
    규격항목 = 규격항목 or []
    묶음 = _묶기(항목들)
    ...
    if 토큰항목 or 규격항목:
        L.append("| 규정에 어긋난 값(토큰 밖) | %d 가지 |" % len(토큰항목))
        L.append("| 컴포넌트 규격 | %d 개 |" % len(규격항목))
    L.append("")
    ...
    _규정본문(L, 장번호, 토큰항목, 규격항목, 알아봄들, 여러화면)
    ...
    if 항목들 or 구조들 or 토큰항목 or 규격항목:
        ...
        for it in 토큰항목:
            앞 = ("%s " % it["화면"]["키"]) if 여러화면 else ""
            L.append("- [ ] `%s` %s%s `%s` → `var(%s)` (%d곳)"
                     % (it["번호"], 앞, it["css"], it["지금"], it["토큰"], len(it["곳들"])))
        for it in 규격항목:
            앞 = ("%s " % it["화면"]["키"]) if 여러화면 else ""
            L.append("- [ ] `%s` %s%s %s — %s" % (it["번호"], 앞, ("`%s` " % it["고르개"]) if it["고르개"] else "",
                                                  it["규격"], "; ".join(r["무엇"] for r in it["줄"])))
        L.append("")
...
def 지시서(결과, 화면이름="", 차수=None, 정본커밋=None, 화면키=None, 주소=None, 규정=None):
    ...
    L = []
    _머리(L, "개발화면 수정 요청 — %s" % 정보["이름"], [정보], 차수, _정본판(규정, 정본커밋))
...
def 지시서묶음(화면별, 제목="개발화면 수정 요청", 차수=None, 정본커밋=None):
    ...
    for 결과, 메모 in 화면별:
        ...
        if 규정:
            ...
            알아봄들.append(규정["알아봄"])
    L = []
    _머리(L, "%s — 화면 %d개" % (제목, len(화면들)), 화면들, 차수, _정본판(첫규정, 정본커밋))
    _본문(L, 항목들, 구조들, 주의들, 여러화면=True, 토큰항목=토큰항목, 규격항목=규격항목, 알아봄들=알아봄들)
    _꼬리(L, 화면들)
    return "\n".join(L) + "\n"
