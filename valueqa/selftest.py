...
def main():
    ...
    묶음글 = 지시서묶음([(결과, {"화면키": "A-001", "화면이름": "첫 화면", "주소": "https://example.local/a"}),
                    (결과, {"화면키": "B-002", "화면이름": "둘째 화면", "주소": "https://example.local/b"})], 차수=1)
    봄("묶음에 화면 둘 다 나옴", "A-001" in 묶음글 and "B-002" in 묶음글)
    봄("묶음에 '한꺼번에 고칠 것' 이 생김", "한꺼번에 고칠 것" in 묶음글)
    봄("한꺼번에 칸에 두 화면 번호가 같이 적힘",
      "`A-001-#01`" in 묶음글.split("## 2.")[0] and "`A-001-#07`" in 묶음글.split("## 2.")[0])

    # 같은 주소의 상태 화면들은 고칠 곳이 하나다 — 상태 수만큼 되풀이하지 않는다
    상태글 = 지시서묶음([(결과, {"화면키": "S-001", "화면이름": "로그인", "주소": "https://example.local/login"}),
                    (결과, {"화면키": "S-002", "화면이름": "로그인_커서", "주소": "https://example.local/login"})], 차수=1)
    봄("같은 페이지의 상태들은 한 줄로 합쳐짐",
      상태글.count("`S-001-#01`") >= 1 and "S-002-#" not in 상태글)
    봄("합친 줄에 '어느 상태에서 보였는지' 가 적힘", "모든 상태" in 상태글 or "`S-002`" in 상태글)
    봄("색을 HEX 로 적음", "#" in 상태글 and "rgb(" not in 상태글.split("## 이번에 다루지 않은 것")[0])
