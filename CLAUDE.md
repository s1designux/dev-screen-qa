...
## 5. 빌드 순서 (현재 단계: **이미지 검수 플러그인 1.0 — 승인된 병행 조각**)
...
**다음 작업 대기열 (우선순위 순):**
0. **현재 진행:** `plugin-image-qa/` 1.0 — 모양 기반 화면 연결 / 자동 정렬·차이 후보 /
   번호 중심 패널 / 캔버스 포인팅 / 디자인 원본값 표시. 자동 오류 확정·정확한 개발값 추측은 금지.
1. **차수 연동 2차 조각** — 새 차수 추가 + 이전 지적 끌고와 판단
   (고쳐짐=해결후보 / 안 고쳐짐=미해결 유지 / 신규 추가). **사람이 판단, 자동비교 아님.**
   지금은 데모 시드된 1·2차를 '보기'만 되고, 실제로 새 차수를 만드는 흐름이 없다.
2. **실제 검수 데이터 더 넣기** — 지금 한 화면(15건)뿐. 여러 화면·페이지로 탭·카운트·핀 검증.
3. **A4 반출 본구현** — 현재 버튼만 스텁(`report.py` 미변경).
4. **오류 수정방법 값 채우기** (목표값·디자인 토큰).
5. **차수 비교 뷰**(1차↔2차 나란히), **포털에서 직접 네모 치기**, **버전 확정·발행**(17번 정책).
6. **(별도 설치)** 웹 자동 캡처 / 앱 캡처 가이드(20번 방침) / 자동 비교.
