...
- ✅ **승인된 예외(2026-09-01):** `plugin-image-qa/`의 개발화면 이미지 검수 플러그인 1.0.
  여러 디자인 프레임·개발 캡처를 모양으로 연결하고, 자동 정렬·차이 후보·번호 포인팅·
  살아 있는 Figma 원본값 표시까지 만든다. 후보는 사람이 확정하며 Figma는 원본 DB가 아니다.
  기존의 "포털 이슈 → Figma 보드 역생성" 플러그인은 여전히 나중 단계다.
...
- **2026-09-01 사용자 승인:** 포털을 원본으로 유지한 채 `plugin-image-qa/` 1.0 구현을 현재 조각으로 전환.
  기존 포털·값 검수 플러그인을 덮어쓰지 않고 별도 폴더에서 만든다.
...
- 규칙의 **알고리즘**은 검수기 파일(`plugin-image-qa/ui.html`)에, **값**(켜고 끄기·기준값·예외)은 포털 DB `policy_rule`에.
...
- **검수기 ↔ 포털 배선 (2026-09-14 완료, `mvp0/policy_api.py`):** 검수기는 비교 전에
