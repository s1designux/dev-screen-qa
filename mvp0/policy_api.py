"""(사용 안 함 · 2026-09-14) 검수기 플러그인 ↔ 포털 규칙 배선.

**피그마 검수기 플러그인을 더 이상 쓰지 않기로 했다(CLAUDE.md 4번).** 검수는 포털에서만 한다.
이 파일은 지우지 않고 남겨 두지만(이력), 부르는 쪽이 없다. 규칙 값은 포털 화면(/policy…)에서 정한다.

검수기 엔진(engine/ui.html)은 포털이 돌리지만, 규칙 값의 **원본은 포털**이다(CLAUDE.md 8번).
