...
def detail(store,plan,case,notice='',sel_round=None):
    ...
    dialog=f'''<dialog id="capture-picker"><div class="s1-modal-inset"><div class="dialog-head"><h2>디자인·개발 대조</h2><button type="button" onclick="document.getElementById('capture-picker').close()">닫기</button></div><p class="recommendation-status" role="status">유사한 개발 캡처를 찾고 있습니다…</p>{form(base+'/select',selection_controls+comparison+f'<div class="capture-footer"><button {"disabled" if not caps else ""}>이 개발 화면으로 변경</button></div>')}</div></dialog><script>{Path(__file__).with_name('capture_recommendation.js').read_text()}</script>'''
