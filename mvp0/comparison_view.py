...
.cv-dialog::backdrop{background:#11182766}.cv-dialog header{padding:0 0 12px;display:flex;justify-content:space-between}.cv-parts{display:grid;grid-template-columns:1fr 1fr;gap:16px}.cv-parts canvas{width:100%;height:auto;background:#fafafa;border:1px solid #e5e7eb}.cv-parts p{font-size:12px}
'''
JS = r'''
(function(){
 ...
 tools.innerHTML='<button type="button" data-mode="side" aria-pressed="true">나란히</button><button type="button" data-mode="over" aria-pressed="false">겹쳐보기</button><button type="button" data-mode="crop" aria-pressed="false">부분 확대</button><label hidden>디자인 농도 <input aria-label="디자인 농도" type="range" min="0" max="100" value="50"></label><button type="button" data-reset hidden>위치 초기화</button><span class="cv-help" role="status"></span>';
 ...
 function setMode(m){mode=m;pair.classList.toggle('cv-merged',m==='over');glass.hidden=m==='side';glass.style.cursor=m==='crop'?'crosshair':'move';tools.querySelector('label').hidden=m!=='over';tools.querySelector('[data-reset]').hidden=m!=='over';tools.querySelectorAll('[data-mode]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.mode===m)));help.textContent=m==='over'?'드래그·방향키로 맞추기 · 이 브라우저에 보기 위치만 저장 · 검수 내용은 유지':m==='crop'?'개발 이미지에서 확대할 부분을 드래그하세요.':'번호나 검수 카드를 누르면 해당 부분을 확대합니다.';requestAnimationFrame(paint);}
 tools.querySelectorAll('[data-mode]').forEach(b=>b.onclick=()=>setMode(b.dataset.mode));slider.oninput=paint;tools.querySelector('[data-reset]').onclick=()=>{dx=dy=0;save();paint();};
 function point(e){const r=glass.getBoundingClientRect();return{x:e.clientX-r.left,y:e.clientY-r.top};}
 function crop(box){if(!v.naturalWidth||!d.naturalWidth)return;const x=Math.max(0,box.x),y=Math.max(0,box.y),w=Math.min(v.naturalWidth,box.x+box.w)-x,h=Math.min(v.naturalHeight,box.y+box.h)-y;if(w<3||h<3)return;
 ...
 dialog.querySelectorAll('canvas').forEach((c,i)=>{c.width=outW;c.height=outH;const ctx=c.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,outW,outH);const im=i?v:d,k=i?1:scale,ox=i?0:dx,oy=i?0:dy;ctx.drawImage(im,(x-ox)*k,(y-oy)*k,w*k,h*k,0,0,outW,outH);});if(!dialog.open)dialog.showModal();}
 glass.onpointerdown=e=>{if(e.button!==0)return;glass.focus();glass.setPointerCapture(e.pointerId);start={p:point(e),dx,dy};};
 glass.onpointermove=e=>{if(!start)return;const p=point(e);if(mode==='over'){const f=fit();dx=start.dx+(p.x-start.p.x)/f.s;dy=start.dy+(p.y-start.p.y)/f.s;}else start.end=p;paint();};
 ...
 glass.onpointercancel=()=>{start=null;paint();};glass.onkeydown=e=>{if(mode!=='over')return;const n=e.shiftKey?10:1;const moves={ArrowLeft:[-n,0],ArrowRight:[n,0],ArrowUp:[0,-n],ArrowDown:[0,n]};if(!moves[e.key])return;e.preventDefault();dx+=moves[e.key][0];dy+=moves[e.key][1];save();paint();};
